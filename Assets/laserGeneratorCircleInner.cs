...
public class laserGeneratorCircleInner : MonoBehaviour
{
    [Header("Prefab")]
    ...
    private void GenerateLasers()
    {
        if (laserPrefab == null)
        {
            Debug.LogError("[laserGeneratorCircleInner] 未绑定 laserPrefab，无法生成激光。", this);
            return;
        }

        if (sampleCount <= 0 || radiusInner <= 0f || radiusOuter <= radiusInner)
        {
            Debug.LogWarning("[laserGeneratorCircleInner] 参数非法，无法生成激光。", this);
            return;
        }
        ...
        for (int i = 0; i < sampleCount; i++)
        {
            ...
            GameObject instance = Instantiate(laserPrefab, outerPoint, rotation, transform);
