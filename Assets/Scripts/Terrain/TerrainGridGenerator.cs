...
public class TerrainGridGenerator : MonoBehaviour
{
    ...
    [SerializeField] private float terrainWidth = 1f;
    ...
    { 
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                go.transform.position = transform1.position + (terrainWidth * x * transform1.right) + (terrainWidth * y * transform1.forward);
                builtTerrain.Add(go);
            }
        }
    }
